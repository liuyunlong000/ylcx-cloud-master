...
import com.lcxbs.wz.dao.WzScientificMapper;
...
@Service("wzScientificService")
public class WzScientificServiceImpl extends BaseService<WzScientific, Long> implements WzScientificService {

    protected static Logger logger = null;

    @Resource
    private WzScientificMapper wzScientificMapper;

	@Resource
    private SysUserService sysUserService;

    public WzScientificServiceImpl() {
        super();
        logger = Logger.getLogger(this.getClass());
    }
    
    @Override
    public BaseMapper<WzScientific, Long> getMapper() {
    	DynamicDataSource.setDataSource(DynamicDataSource.DEFAULT_DATA_SOURCE_NAME);//设置为默认数据源
        return wzScientificMapper;
    }
    
    @Override
    public SqlSessionFactory getSqlSessionFactory() {
        return wzScientificMapper.getSqlSessionFactory();
    }
	
	@Override
    public int insert(WzScientific model) {
	...
	@Override
    public int updateSelective(WzScientific model) {
        SysUser currentUser=this.sysUserService.getCurrentUser();
        if(null!=currentUser) {
            model.setUpdatedBy(String.valueOf(currentUser.getNid()));
        }
